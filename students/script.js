...
console.log("table1hash " + table_hash_sv);
console.log("table2hash " + table_hash_svhhp);
...
/*function refresh_table_sv() {
    ...
    ajaxEngine.onreadystatechange = function () {
        if (ajaxEngine.readyState == 4 && ajaxEngine.status == 200) { // OK
            var response = JSON.parse(ajaxEngine.responseText);
...
function refresh_table() {
    // Bắt đầu Ajax
    let ajaxEngine = new XMLHttpRequest(); // Tạo đối tượng Ajax Engine
    ajaxEngine.open("GET", "/students/ajax.php?hash1=" + table_hash_sv + "&hash2=" + table_hash_svhhp, true);
    ajaxEngine.send(null);
    // Xử lý sau khi Ajax trả về
    ajaxEngine.onreadystatechange = function () {
        if (ajaxEngine.readyState == 4 && ajaxEngine.status == 200) { // OK
            ...
            if (table_hash_sv != response['hash1']) {
                table_hash_sv = response['hash1'];
                table_sv.innerHTML = response["table1"];
                // Bật lại pagination
                $('#tablesinhvien').DataTable();
                $('.dataTables_length').addClass('bs-select');
            }
            if (table_hash_svhhp != response['hash2']) {
                table_hash_svhhp = response['hash2'];
                table_svhhp.innerHTML = response["table2"];
                // Bật lại pagination
